...
[Wax](https://minicomp.github.io/wiki/wax/) requires more technical skills than any only tool covered by this resource, but offers a unique and accessible means of learning the basics of web development, metadata creation and management, and plain text editing. Wax isn't a tool or platform so much as a workflow - that can make a configurable, static website with only a folder of images and a spreadsheet of image metadata.&#x20;
...
Wax has a unique feature when it comes to supporting IIIF content. While it’s possible to embed pre-existing IIIF objects in iframes, as on any other website, Wax can also be used to create and serve your own IIIF-compliant images, without a server. Standard tifs and jpegs can be added to the workflow, transformed into IIIF tiles, compiled into multi-page objects, and made accessible in an [OpenSeaDragon viewer](https://openseadragon.github.io/). This allows anyone to host a free website featuring IIIF content, with no underlying infrastructure.
